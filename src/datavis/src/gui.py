...
class Sheet(QMainWindow,From_Main):
    ...
    def smartbag(self):
        ...
        path = QFileDialog.getOpenFileName(self, 'OPEN CSV FILE ', r"/root/Downloads/", "CSV FILES(*.csv)")
        #path = filename[0]
        if path[0]!='':
           self.FileN=path[0]
...
class myCanvas(FigureCanvas):
    ...
    def plot(self,xarray,yarray):
        ...
        self.ax5.plot(xarray[25:],yarray[25:])
        self.ax6.plot(xarray[30:],yarray[30:])
        #self.ax2.plot(xarray[2:],yarray[2:])
        #self.ax3.plot(xarray[3:],yarray[3:])
        #self.ax4.plot(xarray[4:],yarray[4:])
        #self.ax5.plot(xarray[5:],yarray[5:])
        #self.ax6.plot(xarray[6:],yarray[6:])
        #self.ax1.set_xlabel(xarray[0])
        #self.ax1.set_ylabel(yarray[0])
        #self.ax2.set_xlabel(xarray[0])
        #self.ax2.set_ylabel(yarray[0])
        self.draw()
